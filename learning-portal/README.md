...
On the Mac, open http://127.0.0.1:3210 and choose **Connect phone** in the header. Copy the displayed HTTPS address to your phone, then choose **Create pairing code** and enter the code on the phone. Codes expire after five minutes and are single-use. A new code replaces the previous one; hiding a code does not revoke it. Browser sessions expire after twelve hours or portal restart; remote cookies are Secure, HttpOnly and SameSite=Strict. Keep the code out of chat, logs and screenshots. This is one personal notebook, not multi-user accounts.
